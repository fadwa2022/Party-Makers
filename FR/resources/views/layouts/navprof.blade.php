<nav class="relative px-4 py-4 flex justify-between items-center bg-transparent">
    ...
    <ul  class="hidden absolute  left-1/4 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-11">
    ...
    <x-nav-link class="font-medium text-sm text-gray-400 hover:text-gray-500"  href="/profileDJ/{{auth()->id()}}" :active="request()->routeIs('Monprofile')">
        {{ __('DJprofile') }}
    </x-nav-link>
    @endif
        <x-nav-link class="font-medium text-sm text-gray-400 hover:text-gray-500"  :href="route('home')" :active="request()->routeIs('login')">
            {{ __('Contact') }}
        </x-nav-link>
    ...
    <form method="POST" action="{{ route('logout') }}">
        @csrf
        <x-nav-link class="hidden lg:inline-block lg:ml-auto lg:mr-3 py-2 px-6 bg-gray-50 hover:bg-gray-100 text-sm text-black  rounded-xl transition duration-200" :href="route('logout')"
